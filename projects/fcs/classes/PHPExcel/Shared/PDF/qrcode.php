...
if (!class_exists('QRcode', false)) {
	...
	class QRcode {
		...
		* Return block number 1
		 ...
		 * @return array Array of RS values:<ul><li>mm = Bits per symbol;</li><li>nn = Symbols per block;</li><li>FCS_to = log lookup table array;</li><li>index_of = Antilog lookup table array;</li><li>genpoly = Generator polynomial array;</li><li>nroots = Number of generator;</li><li>roots = number of parity symbols;</li><li>fcr = First consecutive root, index form;</li><li>prim = Primitive element, index form;</li><li>iprim = prim-th root of 1, index form;</li><li>pad = Padding bytes in shortened block;</li><li>gfpoly</ul>.
		 */
		...
		protected function init_rs_char($symsize, $gfpoly, $fcr, $prim, $nroots, $pad) {
			...
			$rs['nn'] = (1 << $symsize) - 1;
			$rs['pad'] = $pad;
			$rs['FCS_to'] = array_fill(0, ($rs['nn'] + 1), 0);
			...
			for ($i = 0,$root=$fcr*$prim; $i < $nroots; $i++, $root += $prim) {
				...
				for ($j = $i; $j > 0; --$j) {
					if ($rs['genpoly'][$j] != 0) {
						$rs['genpoly'][$j] = $rs['genpoly'][$j-1] ^ $rs['FCS_to'][$this->modnn($rs, $rs['index_of'][$rs['genpoly'][$j]] + $root)];
					} else {
						$rs['genpoly'][$j] = $rs['genpoly'][$j-1];
					}
				}
				// rs->genpoly[0] can never be zero
				$rs['genpoly'][0] = $rs['FCS_to'][$this->modnn($rs, $rs['index_of'][$rs['genpoly'][0]] + $root)];
			}
		 ...
		 */
		 protected function encode_rs_char($rs, $data, $parity) {
			...
			$FCS_TO =& $rs['FCS_to']; // the address of an array of NN elements to convert Galois field elements in index (log) form to polynomial form
			...
			for ($i=0; $i < ($NN - $NROOTS - $PAD); $i++) {
				...
				if ($feedback != $A0) {
					...
					for ($j=1; $j < $NROOTS; ++$j) {
					$parity[$j] ^= $FCS_TO[$this->modnn($rs, $feedback + $GENPOLY[($NROOTS - $j)])];
					}
				}
				// Shift
				array_shift($parity);
				if ($feedback != $A0) {
					array_push($parity, $FCS_TO[$this->modnn($rs, $feedback + $GENPOLY[0])]);
				} else {
					array_push($parity, 0);
				}
			}
