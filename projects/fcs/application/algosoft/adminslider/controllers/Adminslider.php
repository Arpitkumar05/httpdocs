	...
	************************************************************************/
	public function managedata() 
	{
		...
		$this->front_model->auth_check('view_data');
			...
			$this->layouts->set_title('Manage slider');
	...
	{	
		...
		if($editid):
			...
			$this->front_model->auth_check('add_data');
               ...
               //  $data['CATEGORY'] 			= 	$this->admindata_model->get__all_category(); 
		if($this->input->post('SaveChanges')):
			
			$this->form_validation->set_rules('uploadimage0', 'Image', 'trim|required');
		//	$this->form_validation->set_rules('image_alt', 'Image alt', 'trim|required');
             //  $this->form_validation->set_rules('title', 'lang:Title', 'trim|required');
			//$this->form_validation->set_rules('content', 'lang:Content', 'trim|required');
			//$this->form_validation->set_rules('linktitle', 'lang:Link Title', 'trim|required');
                     //   $this->form_validation->set_rules('linkcontent', 'lang:Link Content', 'trim|required');
			...
			if($this->form_validation->run()): 
				...
				$param['image_alt']			= 	addslashes($this->input->post('image_alt'));
                                $param['title']			=	addslashes($this->input->post('title'));
				$param['content']			=	addslashes($this->input->post('content'));
				$param['linktitle']			=	addslashes($this->input->post('linktitle'));
				$param['linkcontent']			=	addslashes($this->input->post('linkcontent'));
		...
		$this->layouts->set_title('Add/edit slider');
	...
	{ 
		...
		if($file_name): 
			...
			//if($imageInformation[0] >= 1200 && $imageInformation[1] >= 900):
				$this->load->library("upload_crop_img");
				$return_file_name	=	$this->upload_crop_img->_upload_image($file_name,$tmp_name,'slider');
