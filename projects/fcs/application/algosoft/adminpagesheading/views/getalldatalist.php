        ...
        <table class="table table-striped table-bordered table-hover dataTable no-footer dtr-inline" role="grid" aria-describedby="dataTables-example_info" style="width: 100%;" width="100%">
          ...
          <tbody>
            ...
            <tr class="<?php if($i%2 == 0): echo 'gradeA odd'; else: echo 'gradeA even'; endif; ?>" role="row">
             ...
             <td style="width: 100px; white-space: normal;" ><?=stripslashes($ALLDATAINFO['page_name'])?></td>
        
                  <td style="width: 500px; white-space: normal;" ><?=stripslashes($ALLDATAINFO['heading'])?></td>
              ...
              <td><div class="table_action_data">
                  <?php if($edit_data == 'Y'): ?>
                      ...
                      <a href="{BASE_URL}{CURRENT_CLASS}/changestatus/<?=manoj_encript($ALLDATAINFO['page_id'])?>/N" title="Active"class="btn btn-success btn-circle tooltipVal" onclick="location.reload()"><i class="fa fa-check"></i></a>
                      <?php else: ?>
                      <a href="{BASE_URL}{CURRENT_CLASS}/changestatus/<?=manoj_encript($ALLDATAINFO['page_id'])?>/Y" title="Inactve"  class="btn btn-warning btn-circle tooltipVal" onclick="location.reload()"><i class="fa fa-times"></i></a>
                      <?php endif; ?>
                      <a class="btn btn-info btn-circle tooltipVal" href="{BASE_URL}{CURRENT_CLASS}/addeditdata/<?=manoj_encript($ALLDATAINFO['page_id'])?>" title="Edit"><i class="fa fa-pencil"></i></a>
                  <?php endif; ?>
                  <?php if($delete_data == 'Y'): ?>
                  <a class="btn btn-danger btn-circle tooltipVal" href="{BASE_URL}{CURRENT_CLASS}/deletedata/<?=manoj_encript($ALLDATAINFO['page_id'])?>" title="Delete"><i class="fa fa-trash-o"></i></a>
                  <?php endif; ?>
                  </div>
              </td>
              <?php endif; ?>
