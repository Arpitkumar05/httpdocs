	...
	{	
		...
		//	$this->form_validation->set_rules('image_alt', 'Image alt', 'trim|required');
			...
			if($this->form_validation->run()): 
				...
				$param['page_id']			=	addslashes($this->input->post('page_id'));
		...
		$this->layouts->set_title('Add/edit banner');
	...
	{ 
		...
		if($file_name): 
			...
			//if($imageInformation[0] >= 1200 && $imageInformation[1] >= 900):
				$this->load->library("upload_crop_img");
				$return_file_name	=	$this->upload_crop_img->_upload_image($file_name,$tmp_name,'banner');
