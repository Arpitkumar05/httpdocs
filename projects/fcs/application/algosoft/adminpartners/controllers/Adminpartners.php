...
class Adminpartners extends CI_Controller {
	...
	************************************************************************/
	public function managedata() 
	{
		...
		$this->front_model->auth_check('view_data');
			...
			$this->layouts->set_title('Manage partners');
	...
	{	
		...
		if($this->input->post('SaveChanges')):
			...
			$this->form_validation->set_rules('uploadimage0', 'Image', 'trim|required');
                        ...
                        if($this->input->post('show_on')!= 'home'):
			$this->form_validation->set_rules('content', 'lang:Content', 'trim|required');
                        endif;
			$this->form_validation->set_rules('show_on', 'lang:show on page', 'trim|required');
			...
			if($this->form_validation->run()): 
				...
				$param['show_on']			=	addslashes($this->input->post('show_on'));
		...
		$this->layouts->set_title('Add/edit partners');
	...
	{ 
		...
		if($file_name): 
			...
			//if($imageInformation[0] >= 1200 && $imageInformation[1] >= 900):
				$this->load->library("upload_crop_img");
				$return_file_name	=	$this->upload_crop_img->_upload_image($file_name,$tmp_name,'partners');
