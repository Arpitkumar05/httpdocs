...
class Adminproductsection extends CI_Controller {
    ...
    public
            function managedata($productId) {
        if(!$productId):
            redirect(base_url().'siteadmin');
        endif;
          $data['product_id'] 				= 	manoj_decript($productId);
         
          $productName                   =          $this->admindata_model->get_product_data_by_id($data['product_id']);
         $this->session->set_userdata('product_id',$data['product_id']);
                     $data['product']  =    $productName['name'] ;
			$data['error'] 				= 	'';
	...
	************************************************************************/
    ...
    public
        ...
        if ($this->input->post('SaveChanges')):
            ...
            $this->form_validation->set_rules('name', 'lang:Section Name', 'trim|required');
            $this->form_validation->set_rules('content', 'lang:Content', 'trim|required');
              $this->form_validation->set_rules('product_id', 'lang:Products Name', 'trim|required');
            ...
            if ($this->form_validation->run()):
                ...
                redirect(base_url() . $this->router->fetch_class() . '/managedata/'.  manoj_encript($_SESSION['product_id']));
            endif;
        endif;

        $this->layouts->set_title('Add/edit product section');
