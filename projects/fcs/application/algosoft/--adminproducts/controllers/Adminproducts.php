...
class Adminproducts extends CI_Controller {
    ...
    public
        ...
        if ((!$id) and ( $_SESSION['product_id'])):
           
            unset(
                    $_SESSION['product_id'], $_SESSION['product_name']
            );
        endif;
        ...
        $id = manoj_decript(substr($id, 0, -1));
    
        if (($token == 'p')):
          if((!$pageRefreshed)):
            
            $product = $this->admindata_model->get_data_by_id($id);
        ...
        if ($_SESSION['product_name']):
            ...
            $this->layouts->set_title('Manage ' . $_SESSION['product_name'] . 'section ');
            $this->layouts->admin_view('managedata', array(), $data);

        else:

            $this->layouts->set_title('Manage products');
            $this->layouts->admin_view('managedata', array(), $data);
    ...
    function getalldatalist() {
        ...
        if ($_SESSION['product_id']) :
            $wherecon['where'] = 'product_id =' . $_SESSION['product_id'];
            $tbl_name          = 'product_section';
        else:
            $tbl_name = 'products';
        endif;
        $con                  = '';
    ...
    function deletedata() {
        ...
        echo 'SUCCESS_____<div class="alert alert-success alert-dismissable"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">X</button><a href="javascript:void(0);" class="alert-link">Success! </a>' . lang('deletesuccess') . '</div>';
        die;
    }
    ...
    public
        ...
        if ($this->input->post('SaveChanges')):
            ...
            $this->form_validation->set_rules('name', 'lang:Products Name', 'trim|required');
            ...
            if ($this->form_validation->run()):
                ...
                if ($_SESSION['product_id']) :
                    $param['product_id'] = addslashes($this->input->post('product_id'));
                endif;
    ...
    function UplodeImage() {
        ...
        if ($file_name):
            ...
            $this->load->library("upload_crop_img");
            $return_file_name = $this->upload_crop_img->_upload_image($file_name, $tmp_name, 'products');
    ...
    /*     * *********************************************************************
     * * Function name : manageproductdata
     * * Developed By : Manoj Kumar
     * * Purpose  : This function used for manage data
     * * Date : 25 NOVEMBER 2017
     * ********************************************************************** */
    ...
    public
        ...
        $this->layouts->set_title('Manage product section ');
        $this->layouts->admin_view('managedata', array(), $data);
    }
    ...
    function getallproductdatalist() {
        $data['error'] = '';
...
/* End of file Admindfpartners.php */
