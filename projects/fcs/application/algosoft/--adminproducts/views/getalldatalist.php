<form id="Data_Form" name="Data_Form" method="post" action="">
  <div class="dataTables_wrapper form-inline dt-bootstrap no-footer">
      ...
      <div class="col-sm-9">
        <div id="dataTables-example_filter" class="dataTables_filter">
          ...
          <select size="1" name="search_field" id="search_field" class="form-control input-sm">
            ...
            <option value="name" <?php if($searchfield == 'name')echo 'selected="selected"'; ?>><?php if(!$_SESSION['product_id']) :echo'Product'; else:echo'Section'; endif;?></option>
      ...
      <div class="col-sm-12">
        <table class="table table-striped table-bordered table-hover dataTable no-footer dtr-inline" role="grid" aria-describedby="dataTables-example_info" style="width: 100%;" width="100%">
          <thead>
            <tr role="row">
                <th>Sr.No.</th>
                 <th><?php if(!$_SESSION['product_id']) :echo'Product'; else:echo'Section'; endif;?></th>
                  <th><?php if(!$_SESSION['product_id']) :echo'Home Page Image'; else:echo'Image'; endif;?></th>
                   <th><?php if(!$_SESSION['product_id']) :echo'Home Page content'; else:echo'Content'; endif;?></th>
          ...
          <tbody>
           ...
           <td><?=stripslashes($ALLDATAINFO['name'])?></td>
               <td><img src="<?=stripslashes($ALLDATAINFO['image'])?>" alt="<?=stripslashes($ALLDATAINFO['name'])?>" /></td>
                 <td style="width: 400px; white-space: normal;"><?=stripslashes($ALLDATAINFO['content'])?></td>
              ...
              <td><div class="table_action_data">
                  <?php if($edit_data == 'Y'): ?>
                     ...
                     <?php if(!$_SESSION['product_id']) : ?>
                       <a class="btn btn-warning btn-circle tooltipVal" href="{BASE_URL}{CURRENT_CLASS}/Managedata/<?=manoj_encript($ALLDATAINFO['id'])?>p" title="View Product Sections"><i class="fa fa-tags"></i></a>
                       <?php endif; ?>
                  <?php endif; ?>
    ...
    <div class="row mar-top-20">
      ...
      <?php if(!$_SESSION['product_id']):?>
        <a href="javascript:void(0);" title="View Product Sections" class="btn btn-warning btn-circle"><i class="fa fa-pencil"></i></a>&nbsp;->&nbsp;<strong>View Product Sections </strong>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
        <?php endif;
       endif; ?>
      <?php if($delete_data == 'Y'): ?>
