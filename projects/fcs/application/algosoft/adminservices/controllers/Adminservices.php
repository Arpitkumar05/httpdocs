	...
	************************************************************************/
	public function managedata() 
	{
		...
		$this->front_model->auth_check('view_data');
			...
			$this->layouts->set_title('Manage services');
	...
	{	
		...
		if($this->input->post('SaveChanges')):
			...
			$this->form_validation->set_rules('uploadimage0', 'Image', 'trim|required');
                        ...
                        $this->form_validation->set_rules('home_page_content', 'lang:Content', 'trim|required');
                        $this->form_validation->set_rules('name', 'lang:Service Name', 'trim|required');
			...
			if($this->form_validation->run()): 
				...
				$param['home_page_content']			=	addslashes($this->input->post('home_page_content'));
		...
		$this->layouts->set_title('Add/edit services');
	...
	{ 
		...
		if($file_name): 
			...
			//if($imageInformation[0] >= 1200 && $imageInformation[1] >= 900):
				$this->load->library("upload_crop_img");
				$return_file_name	=	$this->upload_crop_img->_upload_image($file_name,$tmp_name,'services');
